...
class SceneObj:
    ...
    def makeSceneNxR(self, tilt, height, pitch, orientation = None, azimuth = 180, nMods = 20, nRows = 7, radname = None):
        ...
        trackerfacing = -1           
#        pitchme=5.6571428571428575
        pitchme = pitch # I don't remember why I did another variable but, oh well.
        linestart=np.floor(nMods/2)
        np.floor(nRows/2)
        ...
        if tilt <= 45: #scan along y facing up/down.
                if abs(np.tan(azimuth*dtor) ) <=1: #(-45 <= (azimuth-180) <= 45) ):  # less than 45 deg rotation in z. still scan y
                    self.frontscan = {'xstart':(-1*self.x/2)+self.x*(1/(sensorsx+1))+(trackerfacing)*self.x*np.floor(nMods/2)+(modw-1)*self.x, 'ystart':  (1/(sensorsy+1))*self.y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor)+pitchme*np.floor(nRows/2)-(pitchme*(rowwanted-1)), 
                    ...
                    self.backscan = {'xstart':(-1*self.x/2)+self.x*(1/(sensorsx+1))+(trackerfacing)*self.x*np.floor(nMods/2)+(modwanted-1)*self.x, 'ystart':  (1/(sensorsy+1))*self.y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor)+pitchme*np.floor(nRows/2)-(pitchme*(rowwanted-1)), 
                                 'zstart': 0.01,
                                 'xinc':self.x*(1/(sensorsx+1)), 'yinc': (1/(sensorsy+1))* self.y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor), 
                                 'zinc':0 , 'Nx': 12, 'Ny':9, 'Nz':1, 'orient':'0 0 1' }
                    ...
                    self.frontscan = {'xstart':(1/(sensorsy+1))*self.y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor)+pitchme*np.floor(nRows/2)-(pitchme*(rowwanted-1)), 'ystart':  (-1*self.x/2)+self.x*(1/(sensorsx+1))+(trackerfacing)*self.x*np.floor(nMods/2)+(modwanted-1)*self.x, 
                                 'zstart': height + self.y *np.sin(tilt*dtor) + 1,
                    ...
                    self.backscan = {'xstart':(1/(sensorsy+1))*self.y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor)+pitchme*np.floor(nRows/2)-(pitchme*(rowwanted-1)), 'ystart':  (-1*self.x/2)+self.x*(1/(sensorsx+1))+(trackerfacing)*self.x*np.floor(nMods/2)+(modwanted-1)*self.x, 
                                 'zstart': 0.01,
                                 'xinc':(1/(sensorsy+1))* self.y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor), 'yinc': self.x*(1/(sensorsx+1)), 
                                 'zinc':0 , 'Nx': 9, 'Ny':12, 'Nz':1, 'orient':'0 0 1' }
        ...
        if tilt <= 45: #scan along y facing up/down.
                if abs(np.tan(azimuth*dtor) ) <=1: #(-45 <= (azimuth-180) <= 45) ):  # less than 45 deg rotation in z. still scan y
                    self.frontscan2 = {'xstart':(-1*self.x/2)+self.x*(1/(sensorsx+1))+(trackerfacing)*self.x*np.floor(nMods/2)-(modwanted-1)*self.x, 'ystart':  (1/(sensorsy+1))*self.y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor)-pitchme*np.floor(nRows/2)+(pitchme*(rowwanted-1)), 
                                 'zstart': height + self.y *np.sin(tilt*dtor) + 1,
                                 'xinc':self.x*(1/(sensorsx+1)), 'yinc': (1/(sensorsy+1))* self.y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor), 
                                 'zinc':0 , 'Nx': 12, 'Ny':9, 'Nz':1, 'orient':'0 0 -1' }
                    self.backscan2 = {'xstart':(-1*self.x/2)+self.x*(1/(sensorsx+1))+(trackerfacing)*self.x*np.floor(nMods/2)-(modwanted-1)*self.x, 'ystart':  (1/(sensorsy+1))*self.y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor)-pitchme*np.floor(nRows/2)+(pitchme*(rowwanted-1)), 
                                 'zstart': 0.01,
                                 'xinc':self.x*(1/(sensorsx+1)), 'yinc': (1/(sensorsy+1))* self.y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor), 
                                 'zinc':0 , 'Nx': 12, 'Ny':9, 'Nz':1, 'orient':'0 0 1' }
                    ...
                    self.frontscan2 = {'xstart':(1/(sensorsy+1))*self.y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor)-pitchme*np.floor(nRows/2)+(pitchme*(rowwanted-1)), 'ystart':  (-1*self.x/2)+self.x*(1/(sensorsx+1))+(trackerfacing)*self.x*np.floor(nMods/2)-(modwanted-1)*self.x, 
                                 'zstart': height + self.y *np.sin(tilt*dtor) + 1,
                                 'xinc':(1/(sensorsy+1))* self.y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor), 'yinc': self.x*(1/(sensorsx+1)), 
                                 'zinc':0 , 'Nx': 9, 'Ny':12, 'Nz':1, 'orient':'0 0 -1' }
                    self.backscan2 = {'xstart':(1/(sensorsy+1))*self.y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor)-pitchme*np.floor(nRows/2)+(pitchme*(rowwanted-1)), 'ystart':  (-1*self.x/2)+self.x*(1/(sensorsx+1))+(trackerfacing)*self.x*np.floor(nMods/2)-(modwanted-1)*self.x, 
                                 'zstart': 0.01,
                                 'xinc':(1/(sensorsy+1))* self.y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor), 'yinc': self.x*(1/(sensorsx+1)), 
                                 'zinc':0 , 'Nx': 9, 'Ny':12, 'Nz':1, 'orient':'0 0 1' }
                else: # invalid azimuth (?)
