...
#        pitchme=5.6571428571428575
        ...
        if tilt <= 45: #scan along y facing up/down.
                if abs(np.tan(azimuth*dtor) ) <=1: #(-45 <= (azimuth-180) <= 45) ):  # less than 45 deg rotation in z. still scan y
                    self.frontscan = {'xstart':(-1*self.x/2)+self.x*(1/(sensorsx+1))+(trackerfacing)*self.x*np.floor(nMods/2)+(modwanted-1)*self.x, 'ystart':  (1/(sensorsy+1))*self.y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor)+pitchme*np.floor(nRows/2)-(pitchme*(rowwanted-1)), 
