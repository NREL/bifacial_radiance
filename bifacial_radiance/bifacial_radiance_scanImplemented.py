...
class RadianceObj:
    ...
    def analysis1axis(self, trackerdict=None, typeofanalysis = 'centermod', sensorsx=1, sensorsy=9,):
        '''
        ...
        frontWm2 = np.empty(int(sensorsx*sensorsy)) # container for tracking front irradiance across module chord
        backWm2 = np.empty(int(sensorsx*sensorsy)) # container for tracking rear irradiance across module chord
        for theta in trackerdict:
            basename = '1axis_%s'%(theta)
            octfile = trackerdict[theta]['octfile']
            analysis = AnalysisObj(octfile,basename)      
            
            if typeofanalysis == 'centermod':
                print "yes centermod"
                frontscan, backscan = analysis.centralModuleAnalysis2(trackerdict, sensorsx, sensorsy)
                frontscan2=frontscan
                backscan2=backscan
            if typeofanalysis == 'specificModuleAnalysis':
                print "yes specificModuleAnalysis"
                frontscan, backscan, frontscan2, backscan2 = analysis.specificModuleAnalysis2(trackerdict, theta, sensorsx, sensorsy, rowwanted, modwanted)                
            if theta>=0:
                frontscan = frontscan2
                backscan = backscan2
                #frontscan = trackerdict[theta]['scene'].frontscan2
                #backscan = trackerdict[theta]['scene'].backscan2
#            else:
...
#                backscan = trackerdict[theta]['scene'].backscan
            ...
            #print trackerdict[theta]['AnalysisObj']
    ...
    def centralModuleAnalysis(self, trackerdict=None, sensorsx=1, sensorsy=9):
        # analysis of octfile results,
        # for central module of the array.
        # use sensorsx and sensorsy to define number of scanning points accross the module 
        # sensorsy is along the slope of the panel (CW)
        ...
        if tilt <= 45: #scan along y facing up/down.
                if abs(np.tan(azimuth*dtor) ) <=1: #(-45 <= (azimuth-180) <= 45) ):  # less than 45 deg rotation in z. still scan y
                    self.frontscan = {'xstart':(-1*self.x/2)+self.x*(1/(sensorsx+1)), 'ystart':  (1/(sensorsy+1))*self.y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor), 
                                 'zstart': height + self.y *np.sin(tilt*dtor) + 1,
                                 'xinc':self.x*(1/(sensorsx+1)), 'yinc': (1/(sensorsy+1))* self.y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor), 
                                 'zinc':0 , 'Nx': 12, 'Ny':9, 'Nz':1, 'orient':'0 0 -1' }
                    self.backscan = {'xstart':(-1*self.x/2)+self.x*(1/(sensorsx+1)), 'ystart':  (1/(sensorsy+1))*self.y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor), 
                                 'zstart': 0.01,
                                 'xinc':self.x*(1/(sensorsx+1)), 'yinc': (1/(sensorsy+1))* self.y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor), 
                                 'zinc':0 , 'Nx': 12, 'Ny':9, 'Nz':1, 'orient':'0 0 1' }
                    ...
                    self.frontscan = {'xstart':(1/(sensorsy+1))*self.y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor), 'ystart':  (-1*self.x/2)+self.x*(1/(sensorsx+1)), 
                                 'zstart': height + self.y *np.sin(tilt*dtor) + 1,
                                 'xinc':(1/(sensorsy+1))* self.y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor), 'yinc': self.x*(1/(sensorsx+1)), 
                                 'zinc':0 , 'Nx': 9, 'Ny':12, 'Nz':1, 'orient':'0 0 -1' }
                    self.backscan = {'xstart':(1/(sensorsy+1))*self.y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor), 'ystart':  (-1*self.x/2)+self.x*(1/(sensorsx+1)), 
                                 'zstart': 0.01,
                                 'xinc':(1/(sensorsy+1))* self.y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor), 'yinc': self.x*(1/(sensorsx+1)), 
                                 'zinc':0 , 'Nx': 9, 'Ny':12, 'Nz':1, 'orient':'0 0 1' }
                ...
                # SIL Haven't modified this yet for MODULE eval.....                
                self.frontscan = {'xstart':0, 'ystart': 0 , 
                             'zstart': height + (1/(sensorsy+1))* self.y *np.sin(tilt*dtor),
                             'xinc':0, 'yinc': 0, 
                             'zinc':(1/(sensorsy+1))* self.y * np.sin(tilt*dtor), 'Nx': 1, 'Ny':1, 'Nz':9, 'orient':'%s %s 0'%(-1*np.sin(azimuth*dtor), -1*np.cos(azimuth*dtor)) }
                self.backscan = {'xstart':self.y * -1*np.sin(azimuth*dtor), 'ystart': self.y * -1*np.cos(azimuth*dtor), 
                             'zstart': height + (1/(sensorsy+1))* self.y *np.sin(tilt*dtor),
                             'xinc':0, 'yinc':0, 
                             'zinc':(1/(sensorsy+1))* self.y * np.sin(tilt*dtor), 'Nx': 1, 'Ny':1, 'Nz':9, 'orient':'%s %s 0'%(np.sin(azimuth*dtor), np.cos(azimuth*dtor)) }

        #Sil: Instead of proofing on the axis to see which type of analysis the user is requestig, I'm bypassing it here.
        # by copying frontscan2.
        # At some point change this.
        self.frontscan2 = selffrontscan
        self.backscan2 = selfbackscan
#        return frontscan, backscan

    def specificModuleAnalysis(self, trackerdict=None, sensorsx=1, sensorsy=9, rowwanted=1, modwanted=1):
        # analysis of octfile results,
        # for any module of the array.
        # use sensorsx and sensorsy to define number of scanning points accross the module 
        # sensorsy is along the slope of the panel (CW)
        
        trackerfacing = -1
        # FRONTSCAN: tilt < 0, morning.
        # FRONTSCAN2: Tilt >=0, afternoon.
        if self.tilt <= 45: #scan along y facing up/down.
                if abs(np.tan(azimuth*dtor) ) <=1: #(-45 <= (azimuth-180) <= 45) ):  # less than 45 deg rotation in z. still scan y
                    self.frontscan = {'xstart':(-1*self.x/2)+self.x*(1/(sensorsx+1))+(trackerfacing)*self.x*np.floor(nMods/2)+(modwanted-1)*self.x, 'ystart':  (1/(sensorsy+1))*self.y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor)+sceneDict['pitch']*np.floor(nRows/2)-(sceneDict['pitch']*(rowwanted-1)), 
                                 'zstart': height + self.y *np.sin(tilt*dtor) + 1,
                                 'xinc':self.x*(1/(sensorsx+1)), 'yinc': (1/(sensorsy+1))* self.y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor), 
                                 'zinc':0 , 'Nx': 12, 'Ny':9, 'Nz':1, 'orient':'0 0 -1' }
                    self.backscan = {'xstart':(-1*self.x/2)+self.x*(1/(sensorsx+1))+(trackerfacing)*self.x*np.floor(nMods/2)+(modwanted-1)*self.x, 'ystart':  (1/(sensorsy+1))*self.y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor)+sceneDict['pitch']*np.floor(nRows/2)-(sceneDict['pitch']*(rowwanted-1)), 
                                 'zstart': 0.01,
                                 'xinc':self.x*(1/(sensorsx+1)), 'yinc': (1/(sensorsy+1))* self.y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor), 
                                 'zinc':0 , 'Nx': 12, 'Ny':9, 'Nz':1, 'orient':'0 0 1' }
                    ...
                    self.frontscan = {'xstart':(1/(sensorsy+1))*self.y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor)+sceneDict['pitch']*np.floor(nRows/2)-(sceneDict['pitch']*(rowwanted-1)), 'ystart':  (-1*self.x/2)+self.x*(1/(sensorsx+1))+(trackerfacing)*self.x*np.floor(nMods/2)+(modwanted-1)*self.x, 
                                 'zstart': height + self.y *np.sin(tilt*dtor) + 1,
                                 'xinc':(1/(sensorsy+1))* self.y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor), 'yinc': self.x*(1/(sensorsx+1)), 
                                 'zinc':0 , 'Nx': 9, 'Ny':12, 'Nz':1, 'orient':'0 0 -1' }
                    self.backscan = {'xstart':(1/(sensorsy+1))*self.y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor)+sceneDict['pitch']*np.floor(nRows/2)-(sceneDict['pitch']*(rowwanted-1)), 'ystart':  (-1*self.x/2)+self.x*(1/(sensorsx+1))+(trackerfacing)*self.x*np.floor(nMods/2)+(modwanted-1)*self.x, 
                                 'zstart': 0.01,
                                 'xinc':(1/(sensorsy+1))* self.y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor), 'yinc': self.x*(1/(sensorsx+1)), 
                                 'zinc':0 , 'Nx': 9, 'Ny':12, 'Nz':1, 'orient':'0 0 1' }
                else: # invalid azimuth (?)
        ...
        # creating a second scaning option for TRACKERS
        trackerfacing = 1
        if self.tilt <= 45: #scan along y facing up/down.
                if abs(np.tan(azimuth*dtor) ) <=1: #(-45 <= (azimuth-180) <= 45) ):  # less than 45 deg rotation in z. still scan y
                    self.frontscan2 = {'xstart':(-1*self.x/2)+self.x*(1/(sensorsx+1))+(trackerfacing)*self.x*np.floor(nMods/2)-(modwanted-1)*self.x, 'ystart':  (1/(sensorsy+1))*self.y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor)-sceneDict['pitch']*np.floor(nRows/2)+(sceneDict['pitch']*(rowwanted-1)), 
                                 'zstart': height + self.y *np.sin(tilt*dtor) + 1,
                                 'xinc':self.x*(1/(sensorsx+1)), 'yinc': (1/(sensorsy+1))* self.y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor), 
                                 'zinc':0 , 'Nx': 12, 'Ny':9, 'Nz':1, 'orient':'0 0 -1' }
                    self.backscan2 = {'xstart':(-1*self.x/2)+self.x*(1/(sensorsx+1))+(trackerfacing)*self.x*np.floor(nMods/2)-(modwanted-1)*self.x, 'ystart':  (1/(sensorsy+1))*self.y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor)-sceneDict['pitch']*np.floor(nRows/2)+(sceneDict['pitch']*(rowwanted-1)), 
                                 'zstart': 0.01,
                                 'xinc':self.x*(1/(sensorsx+1)), 'yinc': (1/(sensorsy+1))* self.y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor), 
                                 'zinc':0 , 'Nx': 12, 'Ny':9, 'Nz':1, 'orient':'0 0 1' }
                    ...
                    self.frontscan2 = {'xstart':(1/(sensorsy+1))*self.y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor)-sceneDict['pitch']*np.floor(nRows/2)+(sceneDict['pitch']*(rowwanted-1)), 'ystart':  (-1*self.x/2)+self.x*(1/(sensorsx+1))+(trackerfacing)*self.x*np.floor(nMods/2)-(modwanted-1)*self.x, 
                                 'zstart': height + self.y *np.sin(tilt*dtor) + 1,
                                 'xinc':(1/(sensorsy+1))* self.y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor), 'yinc': self.x*(1/(sensorsx+1)), 
                                 'zinc':0 , 'Nx': 9, 'Ny':12, 'Nz':1, 'orient':'0 0 -1' }
                    self.backscan2 = {'xstart':(1/(sensorsy+1))*self.y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor)-sceneDict['pitch']*np.floor(nRows/2)+(sceneDict['pitch']*(rowwanted-1)), 'ystart':  (-1*self.x/2)+self.x*(1/(sensorsx+1))+(trackerfacing)*self.x*np.floor(nMods/2)-(modwanted-1)*self.x, 
                                 'zstart': 0.01,
                                 'xinc':(1/(sensorsy+1))* self.y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor), 'yinc': self.x*(1/(sensorsx+1)), 
                                 'zinc':0 , 'Nx': 9, 'Ny':12, 'Nz':1, 'orient':'0 0 1' }
                else: # invalid azimuth (?)
                ...
                # SIL Haven't modified this yet for MODULE eval.....                
                self.frontscan2 = {'xstart':0, 'ystart': 0 , 
                             'zstart': height + (1/(sensorsy+1))* self.y *np.sin(tilt*dtor),
                             'xinc':0, 'yinc': 0, 
                             'zinc':(1/(sensorsy+1))* self.y * np.sin(tilt*dtor), 'Nx': 1, 'Ny':1, 'Nz':9, 'orient':'%s %s 0'%(-1*np.sin(azimuth*dtor), -1*np.cos(azimuth*dtor)) }
                self.backscan2 = {'xstart':self.y * -1*np.sin(azimuth*dtor), 'ystart': self.y * -1*np.cos(azimuth*dtor), 
                             'zstart': height + (1/(sensorsy+1))* self.y *np.sin(tilt*dtor),
                             'xinc':0, 'yinc':0, 
                             'zinc':(1/(sensorsy+1))* self.y * np.sin(tilt*dtor), 'Nx': 1, 'Ny':1, 'Nz':9, 'orient':'%s %s 0'%(np.sin(azimuth*dtor), np.cos(azimuth*dtor)) }
...
class SceneObj:
    ...
    def makeSceneNxR(self, tilt, height, pitch, orientation = None, azimuth = 180, nMods = 20, nRows = 7, radname = None):
        ...
        self.frontscan2=self.frontscan
        self.backscan2=self.backscan
...
class MetObj:
    ...
    def _getTrackingAngles(self,axis_azimuth = 180, limit_angle = 45, angledelta = 5, axis_tilt = 0, backtrack = True, gcr = 1.0/3.0 ):  # return tracker angle data for the system
            ...
            * 'theta_round' : tracker_theta rounded to the nearest 'angledelta'
            ...
            print datetimetz+pd.Timedelta(minutes = 30)
            print "PAUSE"
...
class AnalysisObj:
    ...
    def centralModuleAnalysis2(self, trackerdict=None, sensorsx=1, sensorsy=9):
        # analysis of octfile results,
        # for central module of the array.
        # use sensorsx and sensorsy to define number of scanning points accross the module 
        # sensorsy is along the slope of the panel (CW)

        tilt = trackerdict[theta]['surf_tilt']
        azimuth = trackerdict[theta]['surf_azm']
        height = trackerdict[theta]['ground_clearance']
        dtor = np.pi/180
                 
        if tilt <= 45: #scan along y facing up/down.
                if abs(np.tan(azimuth*dtor) ) <=1: #(-45 <= (azimuth-180) <= 45) ):  # less than 45 deg rotation in z. still scan y
                    self.frontscan = {'xstart':(-1*x/2)+x*(1/(sensorsx+1)), 'ystart':  (1/(sensorsy+1))*y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor), 
                                 'zstart': height + y *np.sin(tilt*dtor) + 1,
                                 'xinc':x*(1/(sensorsx+1)), 'yinc': (1/(sensorsy+1))* y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor), 
                                 'zinc':0 , 'Nx': 12, 'Ny':9, 'Nz':1, 'orient':'0 0 -1' }
                    self.backscan = {'xstart':(-1*x/2)+x*(1/(sensorsx+1)), 'ystart':  (1/(sensorsy+1))*y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor), 
                                 'zstart': 0.01,
                                 'xinc':x*(1/(sensorsx+1)), 'yinc': (1/(sensorsy+1))* y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor), 
                                 'zinc':0 , 'Nx': 12, 'Ny':9, 'Nz':1, 'orient':'0 0 1' }
                    ...
                    self.frontscan = {'xstart':(1/(sensorsy+1))*y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor), 'ystart':  (-1*x/2)+x*(1/(sensorsx+1)), 
                                 'zstart': height + y *np.sin(tilt*dtor) + 1,
                                 'xinc':(1/(sensorsy+1))* y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor), 'yinc': x*(1/(sensorsx+1)), 
                                 'zinc':0 , 'Nx': 9, 'Ny':12, 'Nz':1, 'orient':'0 0 -1' }
                    self.backscan = {'xstart':(1/(sensorsy+1))*y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor), 'ystart':  (-1*x/2)+x*(1/(sensorsx+1)), 
                                 'zstart': 0.01,
                                 'xinc':(1/(sensorsy+1))* y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor), 'yinc': x*(1/(sensorsx+1)), 
                                 'zinc':0 , 'Nx': 9, 'Ny':12, 'Nz':1, 'orient':'0 0 1' }
                else: # invalid azimuth (?)
                ...
                # SIL Haven't modified this yet for MODULE eval.....                
                self.frontscan = {'xstart':0, 'ystart': 0 , 
                             'zstart': height + (1/(sensorsy+1))* y *np.sin(tilt*dtor),
                             'xinc':0, 'yinc': 0, 
                             'zinc':(1/(sensorsy+1))* y * np.sin(tilt*dtor), 'Nx': 1, 'Ny':1, 'Nz':9, 'orient':'%s %s 0'%(-1*np.sin(azimuth*dtor), -1*np.cos(azimuth*dtor)) }
                self.backscan = {'xstart':y * -1*np.sin(azimuth*dtor), 'ystart': y * -1*np.cos(azimuth*dtor), 
                             'zstart': height + (1/(sensorsy+1))* y *np.sin(tilt*dtor),
                             'xinc':0, 'yinc':0, 
                             'zinc':(1/(sensorsy+1))* y * np.sin(tilt*dtor), 'Nx': 1, 'Ny':1, 'Nz':9, 'orient':'%s %s 0'%(np.sin(azimuth*dtor), np.cos(azimuth*dtor)) }
        ...
        self.frontscan2 = self.frontscan
        self.backscan2 = self.backscan
        frontscan = self.frontscan
        backscan = self.backscan
        backscan2 = self.backscan2
        frontscan = self.frontscan2
        
        return frontscan, backscan, frontscan2, backscan2
        
    def specificModuleAnalysis2(self, trackerdict=None, theta=0, sensorsx=1, sensorsy=9, rowwanted=1, modwanted=1):
        # analysis of octfile results,
        # for any module of the array.
        # use sensorsx and sensorsy to define number of scanning points accross the module 
        # sensorsy is along the slope of the panel (CW)
        ...
        if tilt <= 45: #scan along y facing up/down.
                if abs(np.tan(azimuth*dtor) ) <=1: #(-45 <= (azimuth-180) <= 45) ):  # less than 45 deg rotation in z. still scan y
                    self.frontscan = {'xstart':(-1*x/2)+x*(1/(sensorsx+1))+(trackerfacing)*x*np.floor(nMods/2)+(modwanted-1)*x, 'ystart':  (1/(sensorsy+1))*y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor)+sceneDict['pitch']*np.floor(nRows/2)-(sceneDict['pitch']*(rowwanted-1)), 
                                 'zstart': height + y *np.sin(tilt*dtor) + 1,
                                 'xinc':x*(1/(sensorsx+1)), 'yinc': (1/(sensorsy+1))* y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor), 
                                 'zinc':0 , 'Nx': 12, 'Ny':9, 'Nz':1, 'orient':'0 0 -1' }
                    self.backscan = {'xstart':(-1*x/2)+x*(1/(sensorsx+1))+(trackerfacing)*x*np.floor(nMods/2)+(modwanted-1)*x, 'ystart':  (1/(sensorsy+1))*y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor)+sceneDict['pitch']*np.floor(nRows/2)-(sceneDict['pitch']*(rowwanted-1)), 
                                 'zstart': 0.01,
                                 'xinc':x*(1/(sensorsx+1)), 'yinc': (1/(sensorsy+1))* y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor), 
                                 'zinc':0 , 'Nx': 12, 'Ny':9, 'Nz':1, 'orient':'0 0 1' }
                    ...
                    self.frontscan = {'xstart':(1/(sensorsy+1))*y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor)+sceneDict['pitch']*np.floor(nRows/2)-(sceneDict['pitch']*(rowwanted-1)), 'ystart':  (-1*x/2)+x*(1/(sensorsx+1))+(trackerfacing)*x*np.floor(nMods/2)+(modwanted-1)*x, 
                                 'zstart': height + y *np.sin(tilt*dtor) + 1,
                                 'xinc':(1/(sensorsy+1))* y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor), 'yinc': x*(1/(sensorsx+1)), 
                                 'zinc':0 , 'Nx': 9, 'Ny':12, 'Nz':1, 'orient':'0 0 -1' }
                    self.backscan = {'xstart':(1/(sensorsy+1))*y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor)+sceneDict['pitch']*np.floor(nRows/2)-(sceneDict['pitch']*(rowwanted-1)), 'ystart':  (-1*x/2)+x*(1/(sensorsx+1))+(trackerfacing)*x*np.floor(nMods/2)+(modwanted-1)*x, 
                                 'zstart': 0.01,
                                 'xinc':(1/(sensorsy+1))* y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor), 'yinc': x*(1/(sensorsx+1)), 
                                 'zinc':0 , 'Nx': 9, 'Ny':12, 'Nz':1, 'orient':'0 0 1' }
                else: # invalid azimuth (?)
        ...
        if tilt <= 45: #scan along y facing up/down.
                if abs(np.tan(azimuth*dtor) ) <=1: #(-45 <= (azimuth-180) <= 45) ):  # less than 45 deg rotation in z. still scan y
                    self.frontscan2 = {'xstart':(-1*x/2)+x*(1/(sensorsx+1))+(trackerfacing)*x*np.floor(nMods/2)-(modwanted-1)*x, 'ystart':  (1/(sensorsy+1))*y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor)-sceneDict['pitch']*np.floor(nRows/2)+(sceneDict['pitch']*(rowwanted-1)), 
                                 'zstart': height + y *np.sin(tilt*dtor) + 1,
                                 'xinc':x*(1/(sensorsx+1)), 'yinc': (1/(sensorsy+1))* y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor), 
                                 'zinc':0 , 'Nx': 12, 'Ny':9, 'Nz':1, 'orient':'0 0 -1' }
                    self.backscan2 = {'xstart':(-1*x/2)+x*(1/(sensorsx+1))+(trackerfacing)*x*np.floor(nMods/2)-(modwanted-1)*x, 'ystart':  (1/(sensorsy+1))*y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor)-sceneDict['pitch']*np.floor(nRows/2)+(sceneDict['pitch']*(rowwanted-1)), 
                                 'zstart': 0.01,
                                 'xinc':x*(1/(sensorsx+1)), 'yinc': (1/(sensorsy+1))* y * np.cos(tilt*dtor) / np.cos((azimuth-180)*dtor), 
                                 'zinc':0 , 'Nx': 12, 'Ny':9, 'Nz':1, 'orient':'0 0 1' }
                    ...
                    self.frontscan2 = {'xstart':(1/(sensorsy+1))*y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor)-sceneDict['pitch']*np.floor(nRows/2)+(sceneDict['pitch']*(rowwanted-1)), 'ystart':  (-1*x/2)+x*(1/(sensorsx+1))+(trackerfacing)*x*np.floor(nMods/2)-(modwanted-1)*x, 
                                 'zstart': height + y *np.sin(tilt*dtor) + 1,
                                 'xinc':(1/(sensorsy+1))* y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor), 'yinc': x*(1/(sensorsx+1)), 
                                 'zinc':0 , 'Nx': 9, 'Ny':12, 'Nz':1, 'orient':'0 0 -1' }
                    self.backscan2 = {'xstart':(1/(sensorsy+1))*y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor)-sceneDict['pitch']*np.floor(nRows/2)+(sceneDict['pitch']*(rowwanted-1)), 'ystart':  (-1*x/2)+x*(1/(sensorsx+1))+(trackerfacing)*x*np.floor(nMods/2)-(modwanted-1)*x, 
                                 'zstart': 0.01,
                                 'xinc':(1/(sensorsy+1))* y * np.cos(tilt*dtor) / np.sin((azimuth-180)*dtor), 'yinc': x*(1/(sensorsx+1)), 
                                 'zinc':0 , 'Nx': 9, 'Ny':12, 'Nz':1, 'orient':'0 0 1' }
                else: # invalid azimuth (?)
                ...
                self.frontscan2 = {'xstart':0, 'ystart': 0 , 
                             ...
                             'xinc':0, 'yinc': 0, 
                             'zinc':(1/(sensorsy+1))* y * np.sin(tilt*dtor), 'Nx': 1, 'Ny':1, 'Nz':9, 'orient':'%s %s 0'%(-1*np.sin(azimuth*dtor), -1*np.cos(azimuth*dtor)) }
                self.backscan2 = {'xstart':y * -1*np.sin(azimuth*dtor), 'ystart': y * -1*np.cos(azimuth*dtor), 
                             'zstart': height + (1/(sensorsy+1))* y *np.sin(tilt*dtor),
                             'xinc':0, 'yinc':0, 
                             'zinc':(1/(sensorsy+1))* y * np.sin(tilt*dtor), 'Nx': 1, 'Ny':1, 'Nz':9, 'orient':'%s %s 0'%(np.sin(azimuth*dtor), np.cos(azimuth*dtor)) }
...
if __name__ == "__main__":
    ...
    sensorsy=9.0  # (1/(sensorsy+1)) ---> 0.1
    sensorsx=12.0 # (1/(sensorsx+1))
    rowwanted=1
    modwanted=1
    typeofanalysis='specificModuleAnalysis'  # 'centermod' or 'specificModuleAnalysis'
    x=1.650
    y=1.980
    nRows = 7
    nMods = 7
    ...
    trackerdict = demo2.analysis1axis(trackerdict, typeofanalysis)
#    trackerdict = demo2.specificModuleAnalysis2(trackerdict, theta=0, sensorsx=1, sensorsy=9, rowwanted=1, modwanted=1):
