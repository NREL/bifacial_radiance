...
class RadianceObj:
    ...
    def gendaylitSil(self, month, day, hour, dni, dhi):
        ...
        sky_path = 'skies'
         ...
         #" -L %s %s -g %s \n" %(dni/.0079, dhi/.0079, self.ground.ReflAvg) + \
        skyStr =   ("# start of sky definition for daylighting studies\n"  
            "# location name: Golden LAT: 39.742 LON: -105.179\n"
            "!gendaylit %s %s %s" %(month,day,hour) ) + \
            " -a 39.742 -o -105.179 -m -105 -W %s %s -g %s -O 1 \n" %(dni, dhi, self.ground.ReflAvg) + \
            "skyfunc glow sky_mat\n0\n0\n4 1 1 1 0\n" + \
            "\nsky_mat source sky\n0\n0\n4 0 0 1 180\n" + \
            '\nskyfunc glow ground_glow\n0\n0\n4 ' + \
            '%s ' % (self.ground.Rrefl/self.ground.normval)  + \
    ...
    def makeScene(self, moduletype=None, sceneDict=None):
        ...
        if moduletype is None:
            print('makeScene(moduletype, sceneDict).  Available moduletypes: monopanel, simple_panel, mini_panel' )
            return
...
#        self.sceneRAD = self.scene.makeScene10x3(sceneDict['tilt'],sceneDict['height'],sceneDict['pitch'],sceneDict['orientation'],sceneDict['azimuth'])
        self.sceneRAD = self.scene.makeScene7x3(sceneDict['tilt'],sceneDict['height'],sceneDict['pitch'],sceneDict['orientation'],sceneDict['azimuth'])
...
class SceneObj:
    ...
    def __init__(self,moduletype=None):
        ...
        if moduletype == 'simple_panel':  #next module type
            ...
            self.x = 1  # width of module.
            self.y = 1.5 # height of module.
            self.bifi = 1  # bifaciality of the panel
            self.orientation = 'portrait' #default orientation of the scene
    ...
    def makeScene7x3(self, tilt, height, pitch, orientation = None, azimuth = 180):
        '''
        arrange module defined in SceneObj into a 7 x 3 array
        Valid input ranges: Tilt 0-90 degrees.  Azimuth 45-315 degrees
        ...
        text += '-rx %s -t 0 0 %s ' %(tilt, height)
        # create 10-element array along x, 3 along y
        text += '-a 7 -t %s 0 0 -a 3 -t 0 %s 0 ' %(self.x+ 0.01, pitch) 
        
        # azimuth rotation of the entire shebang
        text += '-i 1 -t %s %s 0 -rz %s ' %(-self.x*2.5, -pitch, 180-azimuth) # was *4
        
        text += self.modulefile
        # save the .RAD file
        
        radfile = 'objects\\%s_%s_%s_7x3.rad'%(self.moduletype,height,pitch)
        with open(radfile, 'wb') as f:
            f.write(text)
...
class AnalysisObj:
    ...
    def ModelComparison(self, octfile, basename):
        # analysis of octfile results based on the SolarWorld architecture (1x1.5m)
        # usable for \objects\PVSC_4array.rad
        # PVSC front view. iterate x = 0.1 to 4 in 26 increments of 0.15. z = 0.9 to 2.25 in 9 increments of .15
        #x = 3.2 - middle of east panel
    ...
    def analysis(self, octfile, basename, frontscan, backscan, plotflag = False):
        ...
        self.saveResults(frontDict, frontDict,'irr_%s.csv'%(basename) )
        return frontDict, backDict; 

    def analysisSil(self, octfile, basename, frontscan, backscan, plotflag = False):

        mylpts='0 0.139941182 0.01 0 0 1 \r0 0.223649841 1.76496576878 0 0 -1 \r0 0.3073585 0.01 0 0 1 \r0 0.391067159 0.01 0 0 1 \r0 0.474775818 1.76496576878 0 0 -1 \r0 0.558484477 0.01 0 0 1'
#Fake points
#123        mylpts='0 0.61 0.01 0 0 1 \r0 1.014 0.01 0 0 1 \r0 1.50 0.01 0 0 1 \r0 1.93 0.01 0 0 1 \r0 2.48 0.01 0 0 1 \r0 0.9 0.01 0 0 1'

        OTFDict = self.irrPlotNew(octfile,mylpts,basename+'_Front',plotflag)          
    
        self.saveResults(OTFDict, OTFDict,'irr_%s.csv'%(basename) )
        return OTFDict;
...
if __name__ == "__main__":
 ...
 #   epwfile = demo.getEPW(37.5,-77.6) #can't run this within NREL firewall.  BOO
    ...
    writefiletitle="C:\Users\Silvana\Documents\RadianceScene\Test\\results\OTF_RADIANCE7.csv"
    OTFDataFile="C:\\Users\\Silvana\\Documents\\BF_BifacialIrradiances\\BF_BifacialIrradiances\\data\\1442_ALLDAYS_RMIS.csv"
    
    data = pd.read_csv(OTFDataFile, skiprows=0, header=0)
    direct_normal = data.loc[:,"direct_normal"]
    diffuse_horizontal = data.loc[:,"diffuse_horizontal"]
    myAxisTitles=data.loc[:,"measdatetime"]
    ambient_temperature=data.loc[:,"ambient_temperature"]
    wind_speed=data.loc[:,"wind_speed"]
    ...
    with open (writefiletitle,'w') as csvfile:
        ...
        outputtitles=['Year', 'Month', 'Day', 'Hour', 'Minute', 'DNI', 'DHI', 'Tamb', 'VWind',
                       'IMT1', 'IMT2', 'IMT3' ,'IMT4', 'IMT5', 'IMT6']
        sw.writerow(outputtitles)

        while rl < 942:
            
            myTimestamp=myAxisTitles[rl]
            hour, minute = myTimestamp.split(':')
            # Please notice that since I deleted hours before 5 AM manually on Excel,
            # seconds somehoe dissapear. If it's the OTF RMIS database file RAW,
            # you need to use the two options below that are commented out
            # hour, minute, second = myTimestamp.split(':')
            #123 2 Do: Add an option to do analysis only for hours with Irrad above a value. (ignore night hours)
            date, hour = hour.split(' ')
...
#            if int(month)==8 and int(day)<21:
#               continue
 #           if int(month)==8 and int(day)==21 and my_hour<15.11:  # Days I already analyzed
  #              continue
   #        if int(month)==8 and int(day)==12 and my_hour<12.4:   # Days I already analyzed
    #            continue
            if my_hour<5.5:           # Avoid sun below horizon before sunrise
                continue
            if my_hour>18.5:          # Avoid sun below horizon after sunset     
                continue
            if int(day)==24 and my_hour>14:  # Ignore data while switching to Height 2
                continue
            if int(day)==25 and my_hour<10:  # Ignore data while switching to Height 2
                continue
            if int(day)>=25:                                   
                C=0.36576   # Height 2
            if int(month)==9 and int(day)==1 and my_hour>13:
                continue
            if int(month)==9 and int(day)>=2:
                C=0.09144 # meters
            if int(month)==9 and int(day)==11 and my_hour>12 and my_hour<13:
                continue
            if int(month)==9 and int(day)>11 and my_hour>=13:
                C=0.09144
                rtr=1.2192
            ...
            sceneDict = {'tilt':10,'pitch':rtr,'height':C,'orientation':'landscape','azimuth':180}
...
#                data_sub['Back/FrontRatio'] = self.backRatio
#                df = pd.DataFrame.from_dict(data_sub)
                outputvalue=[2017, int(month), int(day), int(hour), int(minute), dni, dhi, ambient_temperature[rl], wind_speed[rl]]
